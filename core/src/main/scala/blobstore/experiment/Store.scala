...
import blobstore.experiment.url.{Authority => UrlAuthority, Path, Url}
import cats.MonadError
import fs2.{Pipe, Stream}

abstract class Store[F[_]: MonadError[*[_], Throwable], Scheme <: String, Authority <: UrlAuthority, Blob] {
  ...
  /**
    ...
    * @return sink of bytes
    */
  def putRotate(authority: Authority, computePath: F[Path[Blob]], limit: Long): Pipe[F, Byte, Unit]
