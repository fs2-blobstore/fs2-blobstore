...
object Store {
  ...
  private[blobstore] class DelegatingStore[F[_]: MonadError[*[_], Throwable], Blob](
    ...
    validateInput: Url => Validated[Throwable, Path.Plain] = (_: Url).path.valid[Throwable]
  ) extends Store[F, Blob] {

    override def list(url: Url, recursive: Boolean): Stream[F, Path[Blob]] =
      validateInput(url).liftTo[Stream[F, *]].flatMap(delegate.list(_, recursive))

    override def get(url: Url, chunkSize: Int): Stream[F, Byte] =
      validateInput(url).liftTo[Stream[F, *]].flatMap(delegate.get(_, chunkSize))

    override def put(url: Url, overwrite: Boolean = true, size: Option[Long] = None): Pipe[F, Byte, Unit] =
      s => validateInput(url).liftTo[Stream[F, *]].flatMap(p => s.through(delegate.put(p, overwrite, size)))

    override def move(src: Url, dst: Url): F[Unit] =
      (validateInput(src).toEither, validateInput(dst).toEither).tupled.liftTo[F].flatMap((delegate.move[
        String,
        String
      ] _).tupled)

    override def copy(src: Url, dst: Url): F[Unit] =
      (validateInput(src).toEither, validateInput(dst).toEither).tupled.liftTo[F].flatMap((delegate.copy[
        String,
        String
      ] _).tupled)

    override def remove(url: Url, recursive: Boolean): F[Unit] =
      validateInput(url).liftTo[F].flatMap(delegate.remove(_, recursive))

    override def stat(url: Url): Stream[F, Path[Blob]] =
      validateInput(url).liftTo[Stream[F, *]].flatMap(s => Stream.eval(delegate.stat[String](s)).unNone)

    override def putRotate(computePath: F[Url], limit: Long): Pipe[F, Byte, Unit] = {
      val p = computePath.flatMap(validateInput(_).liftTo[F])
