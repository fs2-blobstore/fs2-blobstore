...
class BoxStoreIntegrationTest extends AbstractStoreTest[BoxPath] {
  ...
  override def beforeAll(): Unit = {
    ...
    val items     = boxClient.folders.getFolderItems(rootFolder.getId)
    items.getEntries.asScala.toList
      .filter { item =>
        val createdAt = if (item.isFileFull) item.getFileFull.getCreatedAt
        else if (item.isFolderFull) item.getFolderFull.getCreatedAt
        else null
        createdAt != null && createdAt.toInstant.atZone(ZoneOffset.UTC).isBefore(threshold)
      }
      .foreach {
        ...
        case item if item.isFileFull =>
          ...
          Try(boxClient.files.deleteFileById(item.getFileFull.getId))
  ...
  it should "expose underlying metadata" in {
    ...
    val paths = boxStore
      .listUnderlying(dirP.path, List.empty, recursive = false)
      .compile
      .toList
      .unsafeRunSync()
