...
class BoxStore[F[_]: Async](
        ...
        info.representation.fileOrFolder match {
          ...
          case Right(folder) =>
            listFolderItems(folder.getId, fields)
              ...
              .unNone
              .map { fileOrFolder =>
                val (pathCollection, itemName) = fileOrFolder match {
                  case Left(file) =>
                    val entries = Option(file.getPathCollection).map(_.getEntries.asScala.toList).getOrElse(Nil)
                    (entries.map(_.getName), file.getName)
                  case Right(f) =>
                    val entries = Option(f.getPathCollection).map(_.getEntries.asScala.toList).getOrElse(Nil)
                    (entries.map(_.getName), f.getName)
                }
                val pathList = pathCollection.dropWhile(_ == "All Files") match {
                  case Nil  => pathCollection.drop(1)
                  case list => list
                }
                val p = Path(pathList.mkString("/")) / itemName
                p.as(BoxPath(fileOrFolder))
              }
        }
  ...
  private def listFolderItems(folderId: String, fields: List[String]): Stream[F, Item] = {
    ...
    Stream.unfoldEval(0L) { offset =>
      Async[F].blocking {
        val builder = new GetFolderItemsQueryParams.Builder()
          .offset(offset)
          .limit(limit)
        val params =
          if (fields.nonEmpty) builder.fields(fields.asJava).build()
          else builder.build()
